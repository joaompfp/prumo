...
def painel_kpi(kpi_id, label, source, indicator, description=None, explain=None, **kwargs):
    ...
    if ann:
        kpi["annotation"] = ann
    # Flag stale indicators (data > 1 year old)
    import re as _re
    _period = kpi.get("period", "")
    _year_match = _re.match(r"(\d{4})", _period)
    if _year_match:
        from datetime import datetime
        _data_year = int(_year_match.group(1))
        if datetime.now().year - _data_year > 1:
            kpi["stale"] = True
    return kpi
...
def build_painel():
    ...
    sections = [
        ...
        # ── 6. Externo ───────────────────────────────────────────────
        {
            ...
            "label": "Externo",
            "kpis": [
                painel_kpi("eur_usd",      "EUR/USD",        "BPORTUGAL", "eur_usd",      invert_sentiment=False, unit_override="USD", description=D["eur_usd"],      explain=E["eur_usd"]),
