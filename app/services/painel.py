...
def build_painel():
    ...
    # Load ALL 376+ indicators from DB and add as extra section
    ...
    try:
        ...
        # Group by category (filter out null categories)
        by_category = {}
        for ind, cat in all_indicators:
            cat = cat or "Sem categoria"
            if cat not in by_category:
                by_category[cat] = []
            by_category[cat].append({"indicator": ind})
        ...
        catalog_section = {
            ...
            "kpis": [
                ...
                for i, (cat, items) in enumerate(sorted(by_category.items()))
            ],
        }
        sections.append(catalog_section)
    except Exception as e:
        print(f"[painel] warning: failed to load full catalog: {e}", flush=True)
