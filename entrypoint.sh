...
# Schedule nightly audit at 03:00 (if crond available)
if command -v crond >/dev/null 2>&1 || command -v cron >/dev/null 2>&1; then
    echo "0 3 * * * cd /app && python scripts/audit_nightly.py >> /data/audit-cron.log 2>&1" | crontab -
    crond 2>/dev/null || cron 2>/dev/null || true
    echo "Nightly audit scheduled at 03:00"
fi
